...
class SpacyModel():
    ...
    def _get_regex(self):
        ...
        bytes_metric = '(({})+({})?{}?)+'.format(magnitud, bytes_, union)
        volume_metric = '(({})+({})?{}?)+'.format(magnitud, volumen, union)
        length_metric = '(({})+({})?{}?)+'.format(magnitud, longitud, union)
        mass_metric = '(({})+({})?{}?)+'.format(magnitud, masa, union)

        bytes_compiled = re.compile(bytes_metric)
        volume_compiled = re.compile(volume_metric)
        length_compiled = re.compile(length_metric)
        mass_compiled = re.compile(mass_metric)


        return [(' <BYTES> ', bytes_compiled), (' <VOLUME> ', volume_compiled), (' <LENGTH> ', length_compiled), (' <MASS> ', mass_compiled)]
    ...
    def tokenize_title(self, titles_data):
        ...
        tokenized_titles = tokenized_titles.progress_apply(self.tokenize)
    ...
    def prepare_data(self, input_data, language, parallel=True):
        ...
        if parallel:
            data['spacy_doc'] = data['new_title'].parallel_apply(lambda x: nlp(x))
        else:
            data['spacy_doc'] = data['new_title'].progress_apply(lambda x: nlp(x))
...
if __name__ == '__main__':
    output_spanish_path = 'melifasttext_span_predict.csv'
    output_portuguese_path = 'melifasttext_port_predict.csv'

    input_f = '/home/lisab/MeLiChallenge/small_products.csv'
    #input_t = '../melidata-challenge/data/splitted_dataset/train_0_target_.csv'

    data_model = SpacyModel(input_f)
    data_model.transform_predict('spanish', output_spanish_path)
    data_model.transform_predict('portuguese', output_portuguese_path)
